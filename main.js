...
app.get("/users/budgets/all", (req, res) => {
  ...
  try {
    jwt.verify(token, SECRET_TOKEN, (err, decoded) => {
      ...
      db.query(
        `SELECT * FROM "user_budget" ub JOIN budgets b ON ub.budget_id = b.budget_id WHERE ub.user_id = ${decoded.id} order by current_money desc`,
        (err, result) => {
          if (err) {
            console.log(err);
            return res.json("Internal Server Error").status(500);
...
app.get("/budgets/:id/transactions", (req, res) => {
  ...
  try {
    jwt.verify(token, SECRET_TOKEN, (err, decoded) => {
      ...
      db.query(
        `SELECT * FROM transactions t WHERE t.budget_id = ${id}`,
        (err, result) => {
          if (err) {
            console.log(err);
            return res.json("Internal Server Error").status(500);
...
app.get("/budgets/:id/users", (req, res) => {
  ...
  try {
    jwt.verify(token, SECRET_TOKEN, (err, decoded) => {
      ...
      db.query(
        `select * from user_budget where budget_id = ${id}`,
        (err, result) => {
          if (err) {
            console.log(err);
            return res.json("Internal Server Error").status(500);
    ...
    db.query(
      ...
      (err, result) => {
        ...
        if (result.rows.length == 0) {
          ...
          db.query(
            `select * from users u join user_budget b on b.user_id = u.id where u.id = $1 and budget_id = $2`,
            [userId, id],
            (err, result_user) => {
              if (err) {
                return res
              ...
              if (result_user.rows.length > 0) {
                ...
                db.query(
                  `insert into public.user_budget(user_id, budget_id) values ($1, $2)`,
                  [userId, id],
                  (err) => {
                    if (err) {
                      return res
...
app.delete("/budgets/:id/users/drop", (req, res) => {
  ...
  jwt.verify(token, SECRET_TOKEN, (err, decoded) => {
    ...
    db.query(
      `delete from user_budget WHERE user_id = $1 and budget_id = $2`,
      [decoded.id, id],
      (err, result) => {
        if (err) {
          console.log(err);
...
app.post("/budgets/:id/transactions", (req, res) => {
  ...
  jwt.verify(token, SECRET_TOKEN, (err, decoded) => {
        ...
        (err, result) => {
          ...
          if (result.rows.length == 0) {
            db.query("rollback", () => {
              return res
                .status(400)
                .json({ message: "There is no such Pooly" });
            });
          }
          ...
          if (currentBalance - amount < 0) {
            ...
            db.query(
              `insert into transactions(budget_id, user_id, amount) values ($1, $2, $3)`,
              [id, decoded.id, amount],
              (err, result) => {
                if (err) {
                  db.query("rollback", () => {
                    console.log(err);
                  ...
                  db.query(
                    `update budgets set current_money = current_money - $1 where budget_id = $2`,
                    [amount, id],
                    (err, result) => {
                      if (err) {
                        db.query("rollback", () => {
    ...
    //     }
    //     if (result.rows[0].current_money - amount < 0) {
    //       return res.status(400).json({ message: "Insufficient balance" });
    //     } else {
    //       db.query(
    //         `insert into public.transactions(budget_id, user_id, amount) values ($1, $2, $3)`,
    //         [id, decoded.id, amount],
    //         (err, result) => {
    //       if (err) {
    //         console.log(err);
    ...
    //           return res
    //             .status(200)
    //             .json({ message: "You succesfully added transaction" });
    //         }
    //       );
    //     }
    //   }
